...
export default class Windows extends Component {
  ...
  openWindow(application) {
    ...
    const windowObject = {
      ...
      updateOtherWindowProperty:(property,value)=>{this.callFnOnOtherWindows(windowID,this.updateWindowProperty,property,value)},
    ...
    setTimeout(()=>this.activateWindow(windowID),1)
  }
  ...
  removeWindow(windowID) {
    this.setState({
      openWindows: this.state.openWindows.filter(w => {
        return w.windowID !== windowID
      })
    })
  }
    ...
    return (
      <div className="Windows" style={{ backgroundImage: `url(${background})` }} onContextMenu={(e) => e.preventDefault()}>
        ...
        <Taskbar
          ...
          callFnOnOtherWindows={this.callFnOnOtherWindows}
        />
